...
if (!css.includes('min-height: 100vh;') || css.indexOf('min-height: 100vh;') > css.indexOf('min-height: 100dvh;')) {
  throw new Error('iOS Safari compatibility requires 100vh fallbacks before 100dvh overrides.');
}
...
if (!html.includes('src="/dist/app.js"') || html.includes('src="/app.js"')) {
  throw new Error('HTML must load the transpiled browser bundle, not raw modern app.js.');
}
...
if (!existsSync('public/dist/app.js')) throw new Error('Build must generate public/dist/app.js for iOS Safari compatibility.');
const bundle = readFileSync('public/dist/app.js', 'utf8');
if (bundle.includes('?.') || bundle.includes('??')) {
  throw new Error('Transpiled browser bundle must avoid fragile optional chaining/nullish syntax for iOS Safari 15-era devices.');
}
if (!bundle.includes('traceballPlayerName')) throw new Error('Transpiled bundle must contain the client app code.');
...
if (!app.includes('function roundRect') || !app.includes('quadraticCurveTo')) {
...
if (app.includes("canvas.addEventListener('click', boardClick);") || !app.includes("canvas.addEventListener('pointerup', boardPointerUp)") || !app.includes("canvas.addEventListener('touchend', boardTouchEnd, { passive: false })") || !app.includes('event.preventDefault()') || !app.includes('lastBoardPointerAt')) {
  throw new Error('Board input must use immediate pointer/touch handlers with prevented synthetic clicks instead of delayed click-only canvas input.');
}
if (!app.includes('pendingOnlineMove') || !app.includes('Move is syncing') || !app.includes('clearPendingOnlineMove')) {
  throw new Error('Online board moves must lock while waiting for the server state echo to avoid stale/delayed taps sending the previous move.');
}
if (!html.includes('id="boardHitLayer"') || !app.includes('renderBoardHitTargets') || !app.includes('className = \'board-hit-target\'') || !app.includes('chooseBoardTarget')) {
  throw new Error('Board legal moves must expose DOM hit targets so touch input does not depend on fragile canvas coordinate taps.');
}
if (css.includes('touch-action: none;') || !css.includes('.board-hit-layer') || !css.includes('pointer-events: none;') || !css.includes('touch-action: manipulation;')) {
  throw new Error('Board must keep pinch zoom available while legal-move buttons handle direct taps.');
}
...
if (!sw.includes('traceball-arena-v23') || !sw.includes('/dist/app.js') || sw.includes("'/app.js'") || !sw.includes('SKIP_WAITING')) throw new Error('PWA service worker must refresh and cache the transpiled app shell for installed iPhone apps.');
