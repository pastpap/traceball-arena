...
if (
  !elmBundle.includes("createSocketBridge") ||
  !/type:\s*["']watch["']/.test(elmBundle) ||
  !elmBundle.includes("traceballElmClientId") ||
  !elmBundle.includes("websocketUrl")
) {
  throw new Error(
    "public/elm.js must bridge the Elm shell to WebSocket watch flow with stable clientId handoff.",
  );
}
if (
  !elmBundle.includes("createBoardAsBlue") ||
  !/type:\s*["']claimSeat["']/.test(elmBundle) ||
  !/type:\s*["']joinWaitingList["']/.test(elmBundle) ||
  !/type:\s*["']leaveWaitingList["']/.test(elmBundle) ||
  !elmBundle.includes("Leave seat / forfeit") ||
  elmBundle.includes("Join Game")
) {
  throw new Error(
    "public/elm.js must expose Phase 5 board-centric seating/waiting-list actions without a generic Join Game flow.",
  );
}
if (/autoJoinSingleVacantSeat\(bridge\)/.test(elmBundle)) {
  throw new Error(
    "Watching a board must stay seatless; Elm shell must not auto-claim the only vacant seat.",
  );
}
if (
  !elmBundle.includes("renderReadOnlyBoard") ||
  !elmBundle.includes("data-elm-board-svg") ||
  !elmBundle.includes('viewBox="0 0 900 1300"') ||
  !elmBundle.includes("data-elm-legal-move") ||
  !elmBundle.includes("data-elm-segment") ||
  !elmBundle.includes("Phase 6C board")
) {
  throw new Error(
    "public/elm.js must render the Phase 6C SVG board with traced segments and context-aware legal-move targets.",
  );
}
if (
  !elmBundle.includes("data-elm-legal-context") ||
  !elmBundle.includes("data-elm-legal-playable") ||
  !elmBundle.includes("own-turn") ||
  !elmBundle.includes("opponent-turn") ||
  !elmBundle.includes("Watching: legal moves are preview only.")
) {
  throw new Error(
    "public/elm.js must expose Phase 6B legal-move context markers for own turn, opponent turn, and watcher previews.",
  );
}
if (
  !elmBundle.includes("submitMoveFromLegalTarget") ||
  !elmBundle.includes("wireBoardMoveTargets") ||
  !/type:\s*["']move["']/.test(elmBundle) ||
  !elmBundle.includes("data-elm-pending-move")
) {
  throw new Error(
    "public/elm.js must support Phase 6C own-turn legal-target move submission with pending-move markers.",
  );
}
if (
  !elmBundle.includes("renderRoundResult") ||
  !elmBundle.includes("data-elm-round-result") ||
  !elmBundle.includes("data-elm-round-actions") ||
  !/type:\s*["']reset["']/.test(elmBundle) ||
  !elmBundle.includes("Continue / New Round")
) {
  throw new Error(
    "public/elm.js must support Phase 6D between-round result UI and new-round reset command.",
  );
}
if (
  !elmBundle.includes("renderDisconnectedSeatRecovery") ||
  !elmBundle.includes("data-elm-disconnected-seat") ||
  !elmBundle.includes("data-elm-disconnect-actions") ||
  !/type:\s*["']freeSeat["']/.test(elmBundle) ||
  !elmBundle.includes("Make seat available")
) {
  throw new Error(
    "public/elm.js must support Phase 7 disconnected-seat recovery UI and freeSeat command.",
  );
}
if (
  !elmBundle.includes("renderBoardList") ||
  !elmBundle.includes("loadBoardList") ||
  !elmBundle.includes("data-elm-board-list") ||
  !elmBundle.includes("data-elm-board-recovery") ||
  !elmBundle.includes("/api/rooms")
) {
  throw new Error(
    "public/elm.js must support Phase 8 board list and expired/not-found recovery UI.",
  );
}
if (
...
if (
  ...
  throw new Error(
    "Game model must support Phase 7 disconnected seat reservation and free-after-grace forfeit handling.",
  );
}
if (
  ...
  throw new Error(
    "Game model must support explicit waiting-list join/leave and remove clients from waiting when seated.",
  );
}
if (
...
if (
  !elmView.includes("viewBoard") ||
  !elmView.includes("Waiting list") ||
  !elmView.includes("Watchers")
) {
  throw new Error(
...
if (css.includes("object-fit: contain"))
  throw new Error(
    "Canvas must not use object-fit: contain because it letterboxes the visual bitmap inside a larger click box.",
  );
if (/#board\s*\{[^}]*max-height:/s.test(css))
  throw new Error(
    "Canvas itself must not be max-height constrained; constrain the board-stage width instead so hit testing and pixels share one box.",
  );
if (
  !css.includes(
  ...
  throw new Error(
    "Mobile/tablet board-stage must shrink by available height while preserving the 720/920 clickable box.",
  );
}
...
if (
  ...
  throw new Error(
    "Mobile navigation/card shells must clip their tab contents inside rounded cards.",
  );
}
if (
...
if (/\.play-status\s*\{[^}]*display:\s*block/s.test(css))
  throw new Error(
    "Play page must use one visible top status line, not a separate play-status banner.",
  );
if (
  !css.includes("white-space: nowrap") ||
  ...
  throw new Error(
    "The single board status line must stay compact and one-line.",
  );
}
if (
...
if (
  ...
  heroHtml.includes('class="actions"')
) {
  throw new Error(
...
if (
  ...
  throw new Error(
    "Server must expose the Elm shell at /elm for direct compatibility links.",
  );
}
if (
  !serverSource.includes("TRACEBALL_FRONTEND") ||
  !serverSource.includes("app.get('/legacy'") ||
  !serverSource.includes("app.get('/legacy/room/:roomId'") ||
  ...
  throw new Error(
    "Phase 9 must make the Elm shell the default app route, preserve /legacy fallback routes, redirect old room links to root board links, and keep a TRACEBALL_FRONTEND=legacy rollback flag.",
  );
}
if (
...
if (
  ...
  throw new Error(
    "Play-page Leave / forfeit button must be selected, wired, and visibility-synced with the Match leave button.",
  );
}
if (
  !html.includes('id="playClaimP1"') ||
  !html.includes('id="playClaimP2"') ||
  !app.includes("playClaimP1: document.querySelector") ||
  !app.includes(
    "els.playClaimP1?.addEventListener('click', () => claimOnlineSeat('p1'))",
  ) ||
  !app.includes("els.playClaimP1?.classList.toggle")
) {
  throw new Error(
    "Play board must expose Join Blue/Join Red controls for mobile watchers, not only the Match tab.",
  );
}
if (
  ...
  throw new Error(
    "Explicit Leave must keep the browser as a watcher and block auto-rejoin until the user presses a join button.",
  );
}
...
const openLobbySection =
  openLobbyStart >= 0 && openLobbyEnd > openLobbyStart
    ? app.slice(openLobbyStart, openLobbyEnd)
    : "";
if (
  !app.includes("cachedBoards = rooms") ||
  !openLobbySection.includes(
    "Board opened for watching. Choose an open seat when you want to play.",
  ) ||
  openLobbySection.includes("claimOnlineSeat(")
) {
  throw new Error(
    "Lobby Watch board must open the board as a watcher only; explicit Join Blue/Join Red buttons claim seats.",
  );
}
if (
  !app.includes("sessionTimeText(entry)") ||
  !app.includes("entry.finalScore") ||
  !app.includes("formatHistoryDate(entry.endedAt") ||
  !app.includes("formatHistoryDate(entry.startedAt")
) {
  throw new Error(
    "Server board session history must show saved session score plus start/end date-times.",
  );
}
if (
  !app.includes(
  ...
  !app.includes("ctx.clip()")
) {
  throw new Error("Goal net mesh must be clipped inside the gate side frames.");
}
if (
  !app.includes("const dirX = p.x ? 1 : -1") ||
  !app.includes("Math.hypot(poleVector.x, poleVector.y)") ||
  !app.includes("ctx.moveTo(baseA.x, baseA.y)") ||
  !app.includes("ctx.lineTo(baseB.x, baseB.y)") ||
  !app.includes("flagNormal.x * dirX < 0")
) {
  throw new Error(
    "Corner flags must keep the original outward-leaning pole positions and hang on the outside face of the angled pole.",
  );
}
if (
  !app.includes("function isGateMouthBouncePoint") ||
  !app.includes("gateBounce ? '#050505'") ||
  !html.includes("black gate-mouth dot") ||
  !html.includes("already a bounce point")
) {
  throw new Error(
    "Gate-mouth center bounce dots must be visually black and explained in the rules.",
  );
}
if (
  !app.includes("const MOVE_HINT_ALPHA = 1") ||
  !app.includes("const MOVE_HINT_FADE_IN_MS = 650") ||
  !app.includes("legalMoveHintStartedAt") ||
  !app.includes("startLegalMoveHintFade") ||
  !app.includes("legalMoveHintColor") ||
  !app.includes(
    "gameMode === 'local' ? currentPlayerColor(game.turn) : '#ffe66d'",
  )
) {
  throw new Error(
    "Legal-move hint circles must fade in once, use blue/red only in local same-screen mode, and keep yellow hints online.",
  );
}
if (
  app.includes("MOVE_HINT_PULSE_MS") ||
  app.includes("animateLegalMoveHints") ||
  app.includes("requestLegalMoveHintFrame") ||
  app.includes("now % MOVE_HINT")
) {
  throw new Error(
    "Legal-move hint circles must not pulse continuously after they have appeared.",
  );
}
if (
  !html.includes(">Incoming game</button>") ||
  html.includes(">Existing game</button>") ||
  !app.includes("setOnlineAction('incoming')") ||
  !app.includes("prefillIncomingInviteFromUrl") ||
  !app.includes("generateRandomPlayerName") ||
  !app.includes("updateInviteVisibility")
) {
  throw new Error(
    "Home Online flow must use Incoming game naming, deep-link to Incoming with prefilled invite input, randomize empty player names, and hide invites outside New game.",
  );
}
if (
  !app.includes("const TURN_MARKER_JUMP_MS = 1400") ||
  !app.includes("const TURN_MARKER_MAX_SCALE = 1.55") ||
  !app.includes("const scale = 1 + Math.sin(Math.PI * t)") ||
  !app.includes("drawTurnMarker(x, y, color, scale)") ||
  !app.includes("startTurnMarkerJump") ||
  !app.includes("drawTurnMarkerJump") ||
  !app.includes("mixPlayerColors")
) {
  throw new Error(
    "Turn gate ball must arc more slowly and grow toward mid-board before shrinking at the other gate.",
  );
}
if (
  ...
  throw new Error(
    "Celebration/confetti animation must be slower so players can understand the result.",
  );
}
if (
  !app.includes(
...
if (
  !app.includes("playerId === 'p2'") ||
  !app.includes("applyBoardTransform") ||
  !app.includes("ctx.rotate(Math.PI)") ||
  !app.includes("boardSpacePoint")
) {
  throw new Error(
    "Board view must rotate the renderer and invert click hit-testing for the second player.",
  );
}
if (
  !app.includes("drawTurnGateBall") ||
  !app.includes("ownGateMarginY") ||
  !app.includes("Math.max(leftPost.x, rightPost.x) + 34")
) {
  throw new Error(
    "Current-turn gate ball marker must be drawn on the right side of the active gate.",
  );
}
if (
  !html.includes('id="onlineMoveTimer"') ||
  !html.includes('id="localMoveTimer"') ||
  !html.includes("5 seconds") ||
  !html.includes("30 seconds")
) {
  throw new Error(
    "Move timer setup must offer off, 5, 10, 15, 20, and 30 second levels for online and local games.",
  );
}
if (
  !app.includes("function drawMoveClock") ||
  !app.includes("function drawSevenSegmentDigit") ||
  !app.includes("turnClockSpot") ||
  !app.includes("Math.min(leftPost.x, rightPost.x) - 54")
) {
  throw new Error(
    "Move timer must render as a retro digital clock on the opposite side of the active gate marker.",
  );
}
if (
  !app.includes("selectedMoveTimerSeconds") ||
  !app.includes("moveTimeLimitSeconds") ||
  !app.includes("expireLocalTurnIfNeeded")
) {
  throw new Error(
    "Client must create rooms/local games with configurable move timers and enforce local timeouts.",
  );
}
if (
  !html.includes('id="pauseOverlay"') ||
  !html.includes('id="playPauseGame"') ||
  !html.includes("⏸") ||
  ...
  throw new Error(
    "Pause UI must include a compact visible Play-page pause control and a blurred board overlay.",
  );
}
if (
...
if (
  !html.includes('id="appMenuButton"') ||
  !html.includes('id="appMenuDropdown"') ||
  !html.includes('id="appContentOverlay"') ||
  !html.includes('data-menu-view="history"') ||
  !html.includes('data-menu-view="rules"') ||
  !html.includes('id="appMenuHistory"') ||
  !html.includes('id="appMenuRules"') ||
  html.includes('id="historyList"') ||
  css.includes(".play-pause-button { display: block;") ||
  !app.includes("openAppContent")
) {
  throw new Error(
    "Mobile app menu must be a dropdown first, then open separate History/Rules content windows outside the Match tab.",
  );
}
if (
  !css.includes("align-items: start;") ||
  !css.includes("padding-top: max(14px, env(safe-area-inset-top));") ||
  css.includes(".app-content-overlay { padding: 10px; align-items: end; }")
) {
  throw new Error(
    "Mobile Rules/History content overlay must open near the top safe area, not as a bottom-aligned sheet.",
  );
}
if (
...
if (
  !html.includes("If time expires, no line is drawn") ||
  !html.includes("each move or bounce gets a fresh clock")
) {
  throw new Error("Rules must explain timer resets and timeout turn-passing.");
}
if (
  !app.includes("drawGatePlayerLabels") ||
  !app.includes("drawGoalMesh") ||
  !app.includes("game.score?.[id]")
) {
  throw new Error(
    "Gate labels, light in-gate mesh, and room score rendering are required.",
  );
}
if (!app.includes("traceballClientId") || !app.includes("clientId });")) {
  throw new Error(
    "Client join messages must include a stable browser client id for reconnects.",
  );
}
if (
  !app.includes("function setReplay(index)") ||
  !app.includes("if (replayIndex === null) {") ||
  !app.includes("syncClockAnimation();") ||
  !app.includes("startLegalMoveHintFade")
) {
  throw new Error(
    "Returning replay to live must restart clock animation and legal-move hint drawing immediately.",
  );
}
if (
  !app.includes("replayIndex >= game.moves.length") ||
  !app.includes("replayIndex = null;")
) {
  throw new Error(
    "Incoming live/new-round state must clear replay mode when the replay cursor is at or beyond the live end.",
  );
}
const drawMoveClockBody = app.slice(
  app.indexOf("function drawMoveClock()"),
  app.indexOf("function drawSevenSegmentNumber"),
);
const shouldAnimateClockBody = app.slice(
  app.indexOf("function shouldAnimateClock()"),
  app.indexOf("function showInvite"),
);
const drawLegalMovesBody = app.slice(
  app.indexOf("function drawLegalMoves()"),
  app.indexOf("function legalMoveHintColor"),
);
if (
  drawMoveClockBody.includes("replayIndex !== null") ||
  shouldAnimateClockBody.includes("replayIndex === null") ||
  drawLegalMovesBody.includes("replayIndex !== null")
) {
  throw new Error(
    "Live countdown and legal-move hints must remain visible/animated while only the board path is replayed.",
  );
}
if (
  ...
  throw new Error(
    "Pause/resume must preserve the remaining turn time instead of resetting the timer.",
  );
}
if (
  !app.includes("pause.remainingMs") ||
  !app.includes("resumeLocalTurnClock")
) {
...
if (
  ...
  !app.includes("confettiUntil") ||
  !app.includes("requestAnimationFrame")
) {
  throw new Error(
    "Client must show a winner overlay and animate confetti over the winner gate.",
  );
}
if (
  !html.includes('id="winnerNewRound"') ||
  !app.includes("winnerNewRound: document.querySelector") ||
  !app.includes("els.winnerNewRound.addEventListener('click', resetRound)") ||
  !css.includes(".winner-new-round") ||
  !css.includes("pointer-events: auto")
) {
  throw new Error(
    "Winner banner must include a clickable New Round button wired to resetRound.",
  );
}
if (
  !html.includes('class="board-stage"') ||
  !html.includes('id="winnerClose"') ||
  !app.includes("winnerClose: document.querySelector") ||
  !app.includes(
    "els.winnerClose.addEventListener('click', dismissWinnerOverlay)",
  ) ||
  !app.includes("dismissedWinnerKey") ||
  !app.includes("function dismissWinnerOverlay")
) {
  throw new Error(
    "Winner modal must be dismissable and centered over the board stage instead of the whole card.",
  );
}
for (const marker of [
  ".board-stage {",
  ...
  if (!css.includes(marker))
    throw new Error(
      `Winner modal must stay centered and bounded for long winner names: missing ${marker}`,
    );
}
if (
  !app.includes("function winnerOwnGateY") ||
  !app.includes("return winnerId === 'p1' ? 12 : 0") ||
  app.includes("const winnerGateY = game.winner === 'p1' ? 0 : 12")
) {
  throw new Error(
    "Winner confetti must anchor to the winner’s own gate, not the gate they scored into.",
  );
}
if (
  !app.includes("persistPlayerName") ||
  !app.includes("traceballPlayerName") ||
  !app.includes("playerNameInput.value = playerName")
) {
  throw new Error(
    "Generic player name must be initialized from and persisted to localStorage.",
  );
}
if (
  !app.includes("parseRoomInput") ||
  !app.includes("joinExistingRoom") ||
  !app.includes("/api/rooms/") ||
  !app.includes("input.length > 200")
) {
  throw new Error(
    "Client must safely parse pasted invite links/codes, cap input length, and check room existence before navigation.",
  );
}
if (
  !app.includes("url.origin !== location.origin") ||
  !app.includes("/^[A-Za-z0-9_-]{6,32}$/")
) {
  throw new Error(
    "Pasted room links must be same-origin and room codes must use a strict allowlist.",
  );
}
if (
  !app.includes("gameMode = 'local'") ||
  !app.includes("setHomeMode") ||
  !app.includes("startLocalGame") ||
  !app.includes("makeLocalMove")
) {
  throw new Error(
    "Client app must support an Online/Local Home selector and local same-screen PvP without WebSockets.",
  );
}
if (!app.includes("return gameMode !== 'local' && playerId === 'p2'")) {
  throw new Error(
    "Local same-screen PvP must keep a static board; only online red-player view may rotate.",
  );
}
...
if (
  !app.includes("Local same-screen PvP") ||
  !app.includes("Players face each other")
) {
  throw new Error(
    "Local PvP UI copy must explain static face-to-face same-screen play.",
  );
}
...
for (const marker of [
  ...
  if (!css.includes(marker))
    throw new Error(
      `Match score numbers must justify inward around the center dash: missing ${marker}`,
    );
}
...
if (
  !server.includes("app.get('/api/rooms/:roomId'") ||
  !server.includes("safeRoomId")
) {
  throw new Error(
    "Server must expose a safe direct room lookup for pasted links/codes.",
  );
}
if (
  !server.includes("applyTurnTimeout") ||
  !server.includes("scheduleRoomTimeout") ||
  !server.includes("moveTimeLimitSeconds")
) {
  throw new Error(
    "Server must enforce online move timers authoritatively and accept room timer configuration.",
  );
}
