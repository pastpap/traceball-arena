...
- `/elm` renders the Phase 6C board shell from fixtures or a live board code query, including an SVG board preview from canonical round state.
...
- Phase 6C keeps the server authoritative: only own-turn legal SVG targets submit `{ type: 'move', to }`, watcher/opponent targets stay inert, and the UI marks a pending move until the next server state arrives.
