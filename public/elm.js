...
function persistPlayerName(name) {
  const value = String(name || "").trim().slice(0, 24) || generateRandomPlayerName();
...
function loadSavedElmLocalRuntime() {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object") return { savedLocalGame: null, savedLocalPaused: false };
    return {
      savedLocalGame: parsed.savedLocalGame && typeof parsed.savedLocalGame === "object" ? parsed.savedLocalGame : null,
...
function saveElmLocalRuntime({ savedLocalGame, savedLocalPaused }) {
  getStorage()?.setItem?.(
    ...
    JSON.stringify({ savedLocalGame: savedLocalGame || null, savedLocalPaused: Boolean(savedLocalPaused) }),
  );
}
...
async function mountElmRuntime(root, { boardCode } = {}) {
  ...
  const connect = (code, reqClientId = clientId, isNew = false) => {
    ...
    ws.onopen = () => {
      ...
      ws.send(JSON.stringify({ type: "watch", roomId: code, clientId: String(reqClientId || clientId || "").trim() }));
      if (isNew) app?.ports?.incomingBoardCreated?.send?.(code);
    };
    ws.onmessage = (e) => {
      try { push(JSON.parse(e.data)); }
      catch { push({ type: "error", error: "malformed websocket message" }); }
    };
    ws.onerror = () => pushStatus("error");
    ws.onclose = () => { if (ws !== sock) return; pushStatus("disconnected"); sock = null; };
  };

  app?.ports?.outgoingClientCommand?.subscribe?.((cmd) => {
    if (!cmd || typeof cmd !== "object") return;
    const t = String(cmd.type || "");
    if (t === "watch") { connect(cmd.roomId, cmd.clientId, false); return; }
    if (t === "persistPlayerName") { persistPlayerName(cmd.name); return; }
    if (t === "persistLocalRuntime") {
      saveElmLocalRuntime({
        savedLocalGame: cmd.localGame && typeof cmd.localGame === "object" ? cmd.localGame : null,
        savedLocalPaused: Boolean(cmd.localPaused),
      });
      return;
    }
    if (t === "persistOnlineMoveTimer") { persistOnlineMoveTimer(cmd.seconds); return; }
    if (t === "updateUrl") {
      if (cmd.url && window.history?.replaceState) window.history.replaceState({}, "", String(cmd.url));
      return;
    }
    if (t === "fetchBoardList") {
    ...
    if (t === "createBoard") {
      ...
      fetch("/api/rooms", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ moveTimeLimitSeconds: sec }) })
        .then((r) => r.ok ? r.json() : Promise.reject(r.status))
        .then((d) => {
          const c = String(d?.roomId || "").trim();
          if (c) connect(c, clientId, true);
          else push({ type: "error", error: "Board creation failed: no board code returned." });
        })
        .catch(() => push({ type: "error", error: "Board creation failed." }));
      return;
    }
    if (!sock || typeof sock.send !== "function") { push({ type: "error", error: "Socket unavailable for command dispatch." }); return; }
...
if (typeof navigator !== "undefined" && "serviceWorker" in navigator) {
  navigator.serviceWorker
    .register("/sw.js")
    .then((r) => { r.update?.().catch?.(() => {}); if (r.waiting) r.waiting.postMessage({ type: "SKIP_WAITING" }); })
    .catch(() => {});
}
