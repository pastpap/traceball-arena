...
describe('Task 4 local same-screen runtime helpers', () => {
  it('creates a local runtime model from player names and timer', () => {
    ...
    const model = shell.createLocalRuntimeModel({ blueName: 'Stefan', redName: 'Alex', moveTimeLimitSeconds: 10 });
    ...
    expect(Array.isArray(model.board.currentSession.round.legalMoves)).toBe(true);
    expect(model.board.currentSession.round.legalMoves.length).toBeGreaterThan(0);
  });

  it('serializes and restores a saved local runtime model', () => {
    const { shell } = loadShell();

    const model = shell.createLocalRuntimeModel({ blueName: 'Blue', redName: 'Red', moveTimeLimitSeconds: 30 });
  ...
  it('applies a legal local move and flips turn', () => {
    ...
    const initial = shell.createLocalRuntimeModel({ blueName: 'Blue', redName: 'Red', moveTimeLimitSeconds: 15 });
    const key = '4,5';
    expect(initial.board.currentSession.round.legalMoves.some((point) => `${point.x},${point.y}` === key)).toBe(true);
