...
describe("traceball rules", () => {
  ...
  it("pauses instead of passing back after two consecutive timeouts", () => {
    ...
    expect(applyTurnTimeout(game, 6000)).toMatchObject({
      ok: true,
      timedOutPlayer: "p1",
      nextPlayer: "p2",
    });
    ...
    expect(game.pause).toMatchObject({
      reason: "idle",
      byPlayerId: "p2",
      resumeTurn: "p2",
    });
  ...
  it("pauses on behalf of one player after that same player times out more than twice in a row", () => {
    ...
    expect(makeMove(game, "p2", { x: 5, y: 6 }, 7000)).toMatchObject({
      ok: true,
      bounce: false,
    });
    expect(game.turn).toBe("p1");
    ...
    expect(applyTurnTimeout(game, 12000)).toMatchObject({
      ok: true,
      timedOutPlayer: "p1",
      nextPlayer: "p2",
    });
    ...
    expect(makeMove(game, "p2", { x: 6, y: 6 }, 13000)).toMatchObject({
      ok: true,
      bounce: false,
    });
    expect(game.turn).toBe("p1");
  ...
  it("resumes paused games with only the remaining turn time, not a fresh timer", () => {
    ...
    expect(
      pauseGame(game, { reason: "manual", byPlayerId: "p1", now: 2000 }).ok,
    ).toBe(true);
    expect(game.status).toBe("paused");
    expect(game.turn).toBe("p1");
    expect(game.turnStartedAt).toBe(null);
  ...
  it("only allows the current turn player to manually pause the game", () => {
    const game = readyGame();
    game.moveTimeLimitMs = 5000;
    game.turnStartedAt = 1000;
    game.turn = "p1";
    ...
    expect(
      pauseGame(game, { reason: "manual", byPlayerId: "p2", now: 2000 }),
    ).toMatchObject({
      ok: false,
      error: expect.stringMatching(/turn it is can pause/i),
    });
    expect(game.status).toBe("playing");
  ...
  it("resumes a two-player idle pause with a fresh clock for the second timed-out player only", () => {
    ...
    expect(applyTurnTimeout(game, 11000)).toMatchObject({
      ok: true,
      timedOutPlayer: "p2",
      paused: true,
    });
    ...
    expect(applyTurnTimeout(game, 17000)).toMatchObject({
      ok: true,
      timedOutPlayer: "p2",
      paused: true,
    });
  ...
  it("reserves a disconnected seat during grace and allows same-client reclaim", () => {
    ...
    expect(
      claimSeat(game, "p2", "Red returns", "red-phone", 20_000),
    ).toMatchObject({ ok: true, playerId: "p2", rejoined: true });
    expect(game.players.p2).toMatchObject({
      status: "active",
      name: "Red returns",
      clientId: "red-phone",
  ...
  it("does not auto-resume a paused game when the same client rejoins", () => {
    const game = createGame("room-test");
    claimSeat(game, "p1", "Blue", "blue-client", 1000);
    claimSeat(game, "p2", "Red", "red-client", 1100);
    expect(
      pauseGame(game, { reason: "manual", byPlayerId: "p1", now: 2000 }).ok,
    ).toBe(true);

    const rejoin = rejoinPlayerByClient(game, "blue-client", 2500);

    expect(rejoin).toMatchObject({ ok: true, playerId: "p1", rejoined: true });
    expect(game.status).toBe("paused");
    expect(game.pause).toMatchObject({ byPlayerId: "p1", resumeTurn: "p1" });
  });
  ...
  it("blocks freeing a disconnected opponent until grace expires, then awards a forfeit point", () => {
    ...
    expect(freeDisconnectedSeat(game, "p1", "p2", 69_999)).toMatchObject({
      ok: false,
    });
