...
async function startServer(port) {
  ...
  for (let i = 0; i < 80; i += 1) {
    ...
    try {
      ...
      if (response.ok)
        return { child, baseUrl, wsUrl: `ws://${HOST}:${port}/ws` };
    } catch {}
...
async function waitFor(client, predicate, label = "message", timeoutMs = 3000) {
  ...
  throw new Error(
    `Timed out waiting for ${client.label} ${label}; saw ${JSON.stringify(client.messages)}`,
  );
}
...
describe("realtime WebSocket main playing flows", () => {
  it("does not disconnect a seated player when an older duplicate socket closes after same-client rejoin", async () => {
    ...
    try {
      ...
      await waitFor(
        p2,
        (msg) => msg.type === "joined" && msg.playerId === "p2",
        "p2 joined",
      );
      await waitFor(
        p2,
        (msg) => msg.type === "state" && msg.game?.status === "playing",
        "active game",
      );
      ...
      expect(p1b.messages.at(-1)?.game?.players?.p1?.status).toBe("active");
    } finally {
  ...
  it("keeps the remaining player seated when the opponent leaves while another same-client socket is active", async () => {
    ...
    try {
      ...
      await waitFor(
        p2,
        (msg) => msg.type === "left" && msg.playerId === "p2",
        "p2 left",
      );
      await new Promise((resolve) => setTimeout(resolve, 150));
  ...
  it("only allows the player who paused to start a new round from the paused state", async () => {
    ...
    try {
      ...
      await waitFor(
        p2,
        (msg) => msg.type === "state" && msg.game?.status === "paused",
        "paused game",
      );
      ...
      await waitFor(
        p2,
        (msg) => msg.type === "error" && /paused/i.test(msg.error),
        "opponent reset rejection",
      );
      ...
      await waitFor(
        ...
        (msg) =>
          msg.type === "state" &&
          msg.game?.status === "playing" &&
          msg.version > 4,
        "pause initiator reset",
      );
  ...
  it("passes the first idle timeout, pauses on the second, and keeps the board listed with both players active", async () => {
    ...
    try {
      ...
      await waitFor(
        p1,
        (msg) => msg.type === "state" && msg.game?.turn === "p2",
        "first timeout passed turn",
        7000,
      );
      await waitFor(
        p1,
        (msg) => msg.type === "state" && msg.game?.status === "paused",
        "second timeout paused",
        7000,
      );
