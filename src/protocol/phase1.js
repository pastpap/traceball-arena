...
function phase1BoardState(game) {
  ...
  const occupied = ["p1", "p2"].filter(
    (id) => game.players?.[id]?.status === "active",
  ).length;
...
function phase1Seat(player, color, now = Date.now()) {
  ...
  if (!player || player.status === "vacant")
    return {
      color,
      state: "Vacant",
      player: null,
...
function phase1CurrentSession(game, boardState, now) {
  if (!["SessionActive", "SessionPaused", "BetweenRounds"].includes(boardState))
    return null;
  const roundState =
    boardState === "BetweenRounds" ? "PendingContinue" : "Active";
  const sessionState =
    boardState === "SessionPaused"
      ? "Paused"
      : boardState === "BetweenRounds"
        ? "BetweenRounds"
        : "Active";
  const configuredTimerMs = Number(game.moveTimeLimitMs || 0);
  const exposeTimer =
    Number.isFinite(configuredTimerMs) && configuredTimerMs >= 0;
  const deadlineAt =
    configuredTimerMs > 0 && Number.isFinite(Number(game.turnStartedAt))
  ...
  return {
    ...
    state: sessionState,
    ...(exposeTimer
      ? { moveTimeLimitSeconds: Math.round(configuredTimerMs / 1000) }
      : {}),
    score: {
    ...
    round: {
      ...
      winner:
        game.winner === "p2" ? "red" : game.winner === "p1" ? "blue" : null,
...
function phase1History(history) {
  return history.map((entry) => ({
    ...
    reason: entry.reason,
    winner:
      entry.winner === "p2" ? "red" : entry.winner === "p1" ? "blue" : null,
    forfeitBy:
      entry.loser === "p2" ? "red" : entry.loser === "p1" ? "blue" : null,
    finalScore: {
