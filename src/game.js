...
export function createGame(roomId, options = {}) {
  ...
  const game = {
    ...
    winner: null,
    endReason: null,
    moveTimeLimitMs: normalizeMoveTimeLimitMs(
      options.moveTimeLimitMs ?? secondsToMs(options.moveTimeLimitSeconds),
...
export function publicGame(game) {
  return {
    ...
    watchers: [],
    waitingList: publicPeople(game.waitingList),
    turn: game.turn,
    ball: game.ball,
...
export function addPlayer(game, name, clientId) {
  ...
  if (isWatcherClient(game, cleanClientId))
    return { ok: false, error: "Choose an open seat to rejoin this board." };
  if (cleanClientId) {
...
export function claimSeat(game, seatId, name, clientId, now = Date.now()) {
  ...
  forgetWatcherClient(game, cleanClientId);
  removeWaitingClient(game, cleanClientId);
  if (bothSeatsActive(game) && game.status === "waiting")
    startSession(game, now);
  markUpdated(game, now);
...
export function rejoinPlayerByClient(game, clientId, now = Date.now()) {
  ...
  const cleanClientId = cleanClient(clientId);
  if (!cleanClientId)
    return { ok: false, error: "Client identity is required." };
  for (const id of ["p1", "p2"]) {
    const player = game.players[id];
    if (player?.clientId !== cleanClientId) continue;
    if (player.status === "vacant")
      return { ok: false, error: "That seat is vacant." };
...
export function joinWaitingList(game, name, clientId, now = Date.now()) {
  ...
  for (const id of ["p1", "p2"]) {
    if (
      game.players[id]?.clientId === cleanClientId &&
      isSeatActive(game.players[id])
    ) {
      return { ok: false, error: "You already occupy a seat on this board." };
    }
  }
  const displayName = cleanPlayerName(name, "watcher");
  const existing = game.waitingList.find(
    (person) => person.clientId === cleanClientId,
  );
  if (existing) {
...
export function leaveWaitingList(game, clientId, now = Date.now()) {
  ...
  game.waitingList = game.waitingList.filter(
    (person) => person.clientId !== cleanClientId,
  );
  if (game.waitingList.length === before)
    return { ok: false, error: "You are not on the waiting list." };
  markUpdated(game, now);
...
export function leavePlayer(game, playerId, now = Date.now()) {
  ...
  if (!isSeatActive(game.players[playerId]))
    return { ok: false, error: "That seat is already vacant." };
  ...
  return {
    ...
    forfeit: historyEntry?.reason === "forfeit",
    historyEntry,
  };
}
...
export function markPlayerDisconnected(game, playerId, now = Date.now()) {
  ...
  if (!isSeatActive(player))
    return { ok: false, error: "That seat is not active." };
...
export function freeDisconnectedSeat(
  game,
  actorPlayerId,
  seatId,
  now = Date.now(),
  ...
  normalizeSeats(game);
  if (!["p1", "p2"].includes(actorPlayerId) || !["p1", "p2"].includes(seatId))
    return { ok: false, error: "Invalid seat." };
  if (actorPlayerId === seatId)
    return { ok: false, error: "You cannot free your own reserved seat." };
  if (!isSeatActive(game.players[actorPlayerId]))
    return {
      ok: false,
  ...
  if (seat?.status !== "disconnected")
    return { ok: false, error: "That seat is not disconnected." };
  if (!Number.isFinite(seat.canBeFreedAt) || now < seat.canBeFreedAt)
    return { ok: false, error: "Disconnect grace has not expired." };

  const wasSessionActive = Boolean(
    game.sessionStartedAt &&
    ["playing", "paused", "finished"].includes(game.status),
  );
  let historyEntry = null;
  if (wasSessionActive) {
    game.score = game.score || { p1: 0, p2: 0 };
  ...
  resetCurrentBoardForNextSession(game, now, {
    autoStart: false,
    preserveScore: true,
  });
  game.status = "waiting";
...
export function leavePlayerAfterOpponentGrace(
  game,
  playerId,
  now = Date.now(),
) {
...
export function makeMove(game, playerId, to, now = Date.now()) {
  if (game.status !== "playing")
    return { ok: false, error: "Game is not playing." };
  if (hasTurnTimedOut(game, now))
    return { ok: false, error: "Time expired.", timeout: true };
  ...
  if (!isOneStep(from, target))
    return { ok: false, error: "Move one point in any of 8 directions." };
  if (!isOnBoardOrGoal(target))
    return { ok: false, error: "Move stays on the pitch or through a gate." };
  if (hasSegment(game, from, target))
    return { ok: false, error: "That line was already used." };
  if (isTracedMarginSegment(from, target))
    return { ok: false, error: "The margin line is already traced." };
  if (isBlockedCornerCut(from, target))
    return { ok: false, error: "Cannot cut through the outside corner." };
  ...
  if (moves.length === 0) {
    ...
    finishGame(
      game,
      winner,
      `${playerName(game, game.turn)} is stuck — ${playerName(game, winner)} wins.`,
    );
  } else {
...
export function legalMoves(game) {
  ...
  for (let dy = -1; dy <= 1; dy += 1) {
    for (let dx = -1; dx <= 1; dx += 1) {
      ...
      if (
        isOnBoardOrGoal(to) &&
        !hasSegment(game, game.ball, to) &&
        !isTracedMarginSegment(game.ball, to) &&
        !isBlockedCornerCut(game.ball, to)
      ) {
        options.push(to);
      }
    }
...
export function startTurnClock(game, now = Date.now()) {
  game.turnStartedAt =
    game.status === "playing" && game.moveTimeLimitMs > 0 ? now : null;
}
...
export function applyTurnTimeout(game, now = Date.now()) {
  ...
  timeoutStreaks[timedOutPlayer] =
    Number(timeoutStreaks[timedOutPlayer] || 0) + 1;
  ...
  if (!canReachTurnChange(game)) {
    ...
    finishGame(
      game,
      winner,
      `${playerName(game, timedOutPlayer)} cannot reach a turn-changing move after timing out — ${playerName(game, winner)} wins.`,
    );
    markUpdated(game, now);
  ...
  if (moves.length === 0) {
    ...
    finishGame(
      game,
      timedOutPlayer,
      `${playerName(game, nextPlayer)} is stuck after ${playerName(game, timedOutPlayer)} timed out — ${playerName(game, timedOutPlayer)} wins.`,
    );
  } else {
    startTurnClock(game, now);
  }
...
export function pauseGame(
  ...
  if (game.status !== "playing")
    return { ok: false, error: "Game is not playing." };
  if (reason === "manual" && byPlayerId && byPlayerId !== game.turn) {
    return {
      ok: false,
      error: "Only the player whose turn it is can pause the game.",
    };
  }
  const elapsed =
    game.moveTimeLimitMs > 0 && Number.isFinite(game.turnStartedAt)
      ? Math.max(0, now - game.turnStartedAt)
      : 0;
  const remainingMs =
    game.moveTimeLimitMs > 0 ? Math.max(0, game.moveTimeLimitMs - elapsed) : 0;
  game.status = "paused";
  game.turnStartedAt = null;
  game.pause = {
    reason,
...
export function startSession(game, now = Date.now()) {
  normalizeSeats(game);
  if (!bothSeatsActive(game))
    return { ok: false, error: "Both seats must be filled before starting." };
  game.status = "playing";
  game.turn = "p1";
  game.ball = { ...START };
...
export function resetCurrentBoardForNextSession(
  game,
  now = Date.now(),
  { autoStart = true, preserveScore = false, preserveSession = false } = {},
) {
  const players = game.players;
  const history = Array.isArray(game.history) ? game.history : [];
  const watcherClientIds = Array.isArray(game.watcherClientIds)
    ? [...game.watcherClientIds]
    : [];
  const waitingList = Array.isArray(game.waitingList)
    ? [...game.waitingList]
    : [];
  ...
  const score = preserveScore
    ? { p1: Number(game.score?.p1 || 0), p2: Number(game.score?.p2 || 0) }
    : { p1: 0, p2: 0 };
  const sessionId = preserveSession
    ? game.sessionId || createSessionId(now)
    : null;
  const sessionStartedAt = preserveSession
    ? game.sessionStartedAt || now
    : null;
...
export function archiveSessionResult(
  game,
  {
    winner,
    loser = winner ? otherPlayer(winner) : null,
    reason = "finished",
    endReason = "",
    now = Date.now(),
  } = {},
  ...
  const finalScore = {
    p1: Number(game.score?.p1 || 0),
    p2: Number(game.score?.p2 || 0),
  };
  const entry = {
...
function publicPlayer(player, id) {
  ...
  const canBeFreedAt = seat.canBeFreedAt ?? null;
  return {
    id: seat.id || id,
    ...
    canBeFreedAt,
    canBeFreed:
      seat.status === "disconnected" && Number.isFinite(canBeFreedAt)
        ? Date.now() >= canBeFreedAt
        : false,
  };
}
...
function publicPeople(list = []) {
  ...
  return list.map((person) => ({
    displayName: String(person?.displayName || person?.name || "Guest").slice(
      0,
      24,
    ),
    joinedAt: Number.isFinite(person?.joinedAt) ? person.joinedAt : null,
  }));
}
...
function normalizeSeats(game) {
  ...
  for (const id of ["p1", "p2"]) {
    ...
    if (!game.players[id].color)
      game.players[id].color = id === "p1" ? "#0b7cff" : "#ff3b30";
...
function cleanPlayerName(name, seatId) {
  return (
    String(name || "")
      .trim()
      .slice(0, 24) ||
    (seatId === "p1" ? "Blue" : seatId === "p2" ? "Red" : "Guest")
  );
}
...
function isWatcherClient(game, clientId) {
  return Boolean(
    clientId &&
    Array.isArray(game.watcherClientIds) &&
    game.watcherClientIds.includes(clientId),
  );
}

function rememberWatcherClient(game, clientId) {
  if (!clientId) return;
  game.watcherClientIds = Array.isArray(game.watcherClientIds)
    ? game.watcherClientIds
    : [];
...
function isOnBoardOrGoal(p) {
  const inMain =
    p.x >= 0 && p.x < WIDTH && p.y > TOP_GOAL_Y && p.y < BOTTOM_GOAL_Y;
  const inGate =
    p.x >= GOAL_X_MIN &&
    p.x <= GOAL_X_MAX &&
    (p.y === TOP_GOAL_Y || p.y === BOTTOM_GOAL_Y);
...
function isTracedMarginSegment(from, to) {
  ...
  const inGateMouth =
    Math.min(from.x, to.x) >= GOAL_X_MIN &&
...
function isBlockedCornerCut(from, to) {
  ...
  const touchesBottomOutside =
    (from.y === HEIGHT - 2 && to.y === HEIGHT - 1) ||
    (from.y === HEIGHT - 1 && to.y === HEIGHT - 2);
  if (
...
function goalForMove(playerId, target) {
  ...
  const scoredOpponentGoal =
    (attacksTop && target.y === TOP_GOAL_Y) ||
    (!attacksTop && target.y === BOTTOM_GOAL_Y);
  if (scoredOpponentGoal)
    return { winner: playerId, reason: `${playerId} scored!` };
